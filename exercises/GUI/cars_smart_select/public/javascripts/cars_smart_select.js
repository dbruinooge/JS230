...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelector('#make_select').addEventListener('change', handleMakeSelect);
  ...
  function handleMakeSelect(event) {
    ...
    if (makeValue === '') {
      ...
      const updatedFiltersObj = filterOptions(makeValue);
      filters.innerHTML = filtersTemplate(updatedFiltersObj);
      document.querySelector('#make_select').value = makeValue;
      document.querySelector('#make_select').addEventListener('change', handleMakeSelect);
    }
  }

  function filterOptions(make) {
    const filtered = cars.filter(car => car.make === make);
    return {
      makes: uniqueValues(cars, 'make'),
      models: uniqueValues(filtered, 'model'),
      prices: uniqueValues(cars, 'price'),
      years: uniqueValues(cars, 'year'),
    };
  }
