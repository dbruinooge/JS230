...
function getQueryString() {
  let nonCreditCardFields = ['first_name', 'last_name', 'email', 'password', 'phone']
    .map(name => {
      return name + '=' + encodeURIComponent(form.elements[name].value);
  }).join('&');

  let creditCard = [...document.getElementsByName('credit_card')]
    .map(element => element.value)
    .join('');

  creditCard = 'credit_card=' + encodeURIComponent(creditCard);
  return nonCreditCardFields + '&' + creditCard;
}
...
form.addEventListener('submit', event => {
  ...
  if (checkFormValidity()) {
    document.querySelector('#query_string').textContent = (getQueryString());
  };
});
